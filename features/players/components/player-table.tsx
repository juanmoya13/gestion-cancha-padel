...
export function PlayerTable({ players, movements, onRefresh }: PlayerTableProps) {
  ...
  const filteredPlayers = useMemo(() => {
    ...
    return players.filter((player) => {
      const matchesSearch = !term || `${player.first_name} ${player.last_name} ${player.phones.map((phone) => phone.phone_number).join(" ")}`.toLocaleLowerCase().includes(term);
      const matchesGender = !genderFilter || player.gender === genderFilter;
      const matchesSkillLevel = !skillLevelFilter || player.skill_level === Number(skillLevelFilter);
      return matchesSearch && matchesGender && matchesSkillLevel;
    });
  }, [genderFilter, players, search, skillLevelFilter]);
  ...
  return <div className="space-y-4">
    <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_12rem_12rem]">
      ...
      <label className="sr-only" htmlFor="gender-filter">Filtrar por género</label>
      <select id="gender-filter" value={genderFilter} onChange={(event) => setGenderFilter(event.target.value)} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
        <option value="">Todos los géneros</option>
        <option value="Masculino">Masculino</option>
        <option value="Femenino">Femenino</option>
      </select>
      <label className="sr-only" htmlFor="skill-level-filter">Filtrar por nivel de habilidad</label>
      <select id="skill-level-filter" value={skillLevelFilter} onChange={(event) => setSkillLevelFilter(event.target.value)} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
        <option value="">Todos los niveles</option>
        {Array.from({ length: 10 }, (_, index) => index + 1).map((level) => <option key={level} value={level}>Nivel {level}</option>)}
      </select>
    </div>
